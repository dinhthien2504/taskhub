<template>
  <div class="admin__content bg-light">
          ...
          <div class="row my-3">
            <div class="col-8">
              ...
              <div class="form-group position-relative mb-3 d-flex align-items-center gap-2 mt-3">
                ...
                <div class="mb-2">
                  <input type="text" class="bg-transparent radius-2 border-0 outline-none w-100 cursor-pointer fs-14"
                    v-model="searchText" @focus="showDropdown = true" @blur="hideDropdownWithDelay" @input="filterUsers"
                    :placeholder="$t('task.select_assignee')" />
                  <ul v-show="showDropdown" class="list-group position-absolute w-100 z-3 radius-2 border"
                    ...
                    <li v-for="user in filteredUsers" :key="user.id"
                      ...
                      <img :src="`${BACKEND_URL}/images/user-default.jpg`" class="rounded-circle me-2" width="24"
          ...
          <div v-for="group in groupedList" :key="group.date" class="mt-4">
            ...
            <div v-for="item in group.items" :key="item.id" class="mt-2">
              <div class="d-flex gap-2">
                <img v-if="item.user.avatar" :src="`${BACKEND_URL}/images/users/${item.user.avatar}`" class="radius-2"
                  width="30" height="30" alt="Hình ảnh">
                <img v-else :src="`${BACKEND_URL}/images/user-default.jpg`" class="radius-2" width="30" height="30"
                  alt="Hình ảnh">
...
const fetchTask = async () => {
  try {
    ...
    console.log(error);
    message.emit('show-message', { title: "Error", text: t('task.fetch_error'), type: "error" })
  }
}
...
const fetchStatuses = async () => {
  try {
    ...
    message.emit('show-message', { title: "Error", text: t('task.status_fetch_error'), type: "error" })
  }
}
...
const fetchUsersByProject = async () => {
  try {
    ...
    message.emit('show-message', { title: "Error", text: t('project.fetch_error'), type: "error" })
  }
}
